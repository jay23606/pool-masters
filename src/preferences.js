...
export const TABLE_PREF_DEFAULTS={size:7,felt:FELTS.green,cue:'classic',lighting:'hall',aimSensitivity:.3}
...
export function saveTablePrefs(prefs,storage=globalThis.localStorage){
 const clean=loadTablePrefs({getItem:key=>({
  ...
  'pool-masters:cue':prefs.cue,'pool-masters:lighting':prefs.lighting,'pool-masters:aim-sensitivity':prefs.aimSensitivity}[key]??null)})
 put(storage,'pool-masters:table-size',clean.size);put(storage,'pool-masters:felt',clean.felt);put(storage,'pool-masters:cue',clean.cue);put(storage,'pool-masters:lighting',clean.lighting);put(storage,'pool-masters:aim-sensitivity',clean.aimSensitivity)
 return clean
}
