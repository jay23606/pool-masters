...
export const TABLE_PREF_DEFAULTS={size:7,felt:FELTS.green,cue:'classic',lighting:'hall'}
...
export function loadTablePrefs(storage=globalThis.localStorage){
 ...
 const felt=get(storage,'pool-masters:felt'),cue=get(storage,'pool-masters:cue'),lighting=get(storage,'pool-masters:lighting')
...
export function saveTablePrefs(prefs,storage=globalThis.localStorage){
 const clean=loadTablePrefs({getItem:key=>({
  ...
  'pool-masters:cue':prefs.cue,'pool-masters:lighting':prefs.lighting}[key]??null)})
 put(storage,'pool-masters:table-size',clean.size);put(storage,'pool-masters:felt',clean.felt);put(storage,'pool-masters:cue',clean.cue);put(storage,'pool-masters:lighting',clean.lighting)
 return clean
}
