...
export const TABLE_PREF_DEFAULTS={size:7,felt:FELTS[DEFAULTS.felt],cue:DEFAULTS.cue,rails:DEFAULTS.rails,lighting:'hall',aimSensitivity:.3,shotCam:true,snap:true}
...
export function loadTablePrefs(storage=globalThis.localStorage){
 ...
 const felt=get(storage,'pool-masters:felt'),cue=get(storage,'pool-masters:cue'),lighting=get(storage,'pool-masters:lighting'),rails=get(storage,'pool-masters:rails'),aimSensitivity=Number(get(storage,'pool-masters:aim-sensitivity'))
...
export function saveTablePrefs(prefs,storage=globalThis.localStorage){
 const clean=loadTablePrefs({getItem:key=>({
  ...
  'pool-masters:cue':prefs.cue,'pool-masters:rails':prefs.rails,'pool-masters:lighting':prefs.lighting,'pool-masters:aim-sensitivity':prefs.aimSensitivity,'pool-masters:shot-cam':prefs.shotCam===false?'0':'1','pool-masters:snap-aim':prefs.snap===false?'0':'1'}[key]??null)})
 put(storage,'pool-masters:table-size',clean.size);put(storage,'pool-masters:felt',clean.felt);put(storage,'pool-masters:cue',clean.cue);put(storage,'pool-masters:rails',clean.rails);put(storage,'pool-masters:lighting',clean.lighting);put(storage,'pool-masters:aim-sensitivity',clean.aimSensitivity);put(storage,'pool-masters:shot-cam',clean.shotCam?'1':'0');put(storage,'pool-masters:snap-aim',clean.snap?'1':'0')
 return clean
}
