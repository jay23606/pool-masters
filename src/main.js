...
async function startPractice(level='league',mode=chosenMode(),career=null){state.aiLevel=level;state.game?.destroy();state.mode='practice';state.room=null;state.opponent={name:career?career.name:`${AI_LEVELS[level].label} AI`};showGame();state.career=career;const house=career?normalizeHouse(null):houseRules();state.raceTo=career?career.race:house.race;state.matchScore={a:0,b:0};$('#game').classList.add('focus');history.replaceState({},'',location.pathname);$('#room-label').textContent=career?`Career · ${career.venue} · ${MODES[mode].label}`:`Unranked practice · ${MODES[mode].label}${houseTag(house)}`;$('#versus').innerHTML=`<span><b>${esc(foyer.player.name)}</b><small>You</small></span><i>vs</i><span><b>${esc(state.opponent.name)}</b><small>${career?`Race to ${career.race}`:'Practice'}</small></span>`;renderMatchScore();renderAiRecord();state.game=new PoolGame({mode,house,renderer:await ensureRenderer(),surface:$('.canvas-wrap'),status:$('#game-status'),groupStatus:$('#groups'),callout:$('#callout'),power:$('#power'),powerOut:$('.shot-controls output'),shoot:$('#shoot'),spinPad:$('#spin'),moveCue:$('#move-cue'),changePocket:$('#change-pocket'),sfx,onReplay:paintReplayBar,onShot:trackShot,aimSensitivity:tablePrefs.aimSensitivity,prefs:tablePrefs,host:true,practice:true,aiLevel:level,send:()=>{},onFinish:result=>{music.duck();sfx.result(result.winner==='a');recordAiResult(result.winner==='a');recordRackPerformance(result);trackRack(result);showRackResult(result);$('#next-rack').hidden=false}});renderMatchScore();$('.call-actions').hidden=true;$('#room-sidebar').hidden=true}
// ---- trophies ----
...
function saveHouse(){
 const h=normalizeHouse({race:Number($('#house-race').value),ballInHand:$('#house-bih').value,breaker:$('#house-breaker').value,straightTo:Number($('#house-straight').value)})
