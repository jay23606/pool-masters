...
async function createRoom(){try{const mode=chosenMode();const room=await foyer.createRoom({name:`${foyer.player.name}'s table`,metadata:{game:'pool',mode,ranked:mode==='8ball',resume_until:resumeExpiry(),match_score:emptyScore(),match_target:MATCH_TARGET,...spectatorMeta(foyer.player.id)},maxPlayers:10,status:'waiting'});await enterRoom(room,'player');$('#room-name').value=room.name;$('#room-dialog').showModal()}catch(e){toast(e.message)}}
async function quickPlay(){
 const mode=chosenMode()
 const rooms=(await foyer.listRooms()).filter(r=>r.metadata?.game==='pool'&&modeOf(r.metadata?.mode)===mode&&!r.metadata?.seats?.b)
...
async function enterRoom(room,intent='player'){
 ...
 state.game=new PoolGame({mode:modeOf(room.metadata?.mode),renderer:await ensureRenderer(),surface:$('.canvas-wrap'),status:$('#game-status'),groupStatus:$('#groups'),callout:$('#callout'),power:$('#power'),powerOut:$('.shot-controls output'),shoot:$('#shoot'),spinPad:$('#spin'),moveCue:$('#move-cue'),changePocket:$('#change-pocket'),sfx,onReplay:paintReplayBar,onShot:trackShot,aimSensitivity:tablePrefs.aimSensitivity,prefs:tablePrefs,host:room.isHost,spectator:state.role!=='host'&&state.role!=='player',practice:false,send:broadcastGame,onSave:persistMatch,onTable:m=>applyTablePrefs(m.size,m.felt,{fresh:false,remote:true}),onRack:()=>$('#next-rack').hidden=true,onFinish:result=>{music.duck();sfx.result(result.winner===state.game?.me);finishRanked(result);recordRackPerformance(result);trackRack(result);showRackResult(result);$('#next-rack').hidden=false}})
...
function renderMatchScore(){const game=state.game;if(!game)return;if(state.mode==='hotseat'){$('#match-score').textContent=`Race to ${MATCH_TARGET} · ${game.nameOf('a')} ${state.matchScore.a}–${state.matchScore.b} ${game.nameOf('b')}`;return}const theirs=game.me==='a'?'b':'a';$('#match-score').textContent=`Race to ${state.raceTo||MATCH_TARGET} · ${state.matchScore[game.me]}–${state.matchScore[theirs]}`}
function showRackResult(result){const game=state.game;if(!game||game.finishedResult===result.round)return;game.finishedResult=result.round;state.matchScore=scoreRack(state.matchScore,result.winner);if(state.mode==='hotseat')return showHotSeatResult(game,result);const winner=matchWinner(state.matchScore,state.raceTo||MATCH_TARGET),won=result.winner===game.me,opponent=state.opponent?.name||'AI Coach';renderMatchScore();if(winner&&winner===game.me)track({type:'match',won:true});$('#result-title').textContent=winner?(winner===game.me?'Match won!':'Match lost'):(won?'Rack won':'Rack lost');$('#result-summary').textContent=`Race to ${state.raceTo||MATCH_TARGET} · ${scoreLine(state.matchScore,game.me,foyer.player.name,opponent)}`;$('#result-next').hidden=Boolean(winner);$('#result-next').textContent=state.mode==='practice'?'Next rack':'Play next rack';$('#result-dialog').showModal();if(state.career&&winner)careerMatchDone(winner===game.me);if(state.room?.isHost)state.room.update({metadata:{...state.room.metadata,match_score:state.matchScore}}).catch(console.warn)}
...
async function startPractice(level='league',mode=chosenMode(),career=null){state.aiLevel=level;state.game?.destroy();state.mode='practice';state.room=null;state.opponent={name:career?career.name:`${AI_LEVELS[level].label} AI`};showGame();state.career=career;state.raceTo=career?career.race:MATCH_TARGET;state.matchScore={a:0,b:0};$('#game').classList.add('focus');history.replaceState({},'',location.pathname);$('#room-label').textContent=career?`Career · ${career.venue} · ${MODES[mode].label}`:`Unranked practice · ${MODES[mode].label}`;$('#versus').innerHTML=`<span><b>${esc(foyer.player.name)}</b><small>You</small></span><i>vs</i><span><b>${esc(state.opponent.name)}</b><small>${career?`Race to ${career.race}`:'Practice'}</small></span>`;renderMatchScore();renderAiRecord();state.game=new PoolGame({mode,renderer:await ensureRenderer(),surface:$('.canvas-wrap'),status:$('#game-status'),groupStatus:$('#groups'),callout:$('#callout'),power:$('#power'),powerOut:$('.shot-controls output'),shoot:$('#shoot'),spinPad:$('#spin'),moveCue:$('#move-cue'),changePocket:$('#change-pocket'),sfx,onReplay:paintReplayBar,onShot:trackShot,aimSensitivity:tablePrefs.aimSensitivity,prefs:tablePrefs,host:true,practice:true,aiLevel:level,send:()=>{},onFinish:result=>{music.duck();sfx.result(result.winner==='a');recordAiResult(result.winner==='a');recordRackPerformance(result);trackRack(result);showRackResult(result);$('#next-rack').hidden=false}});renderMatchScore();$('.call-actions').hidden=true;$('#room-sidebar').hidden=true}
// ---- trophies ----
...
async function startHotSeat(mode=chosenMode()){
 ...
 showGame();state.raceTo=MATCH_TARGET;$('#game').classList.add('focus');history.replaceState({},'',location.pathname)
 $('#room-label').textContent=`Two players · ${MODES[mode].label}`
...
function showHotSeatResult(game,result){
 ...
 $('#result-summary').textContent=`Race to ${MATCH_TARGET} · ${game.nameOf('a')} ${state.matchScore.a} — ${state.matchScore.b} ${game.nameOf('b')}`
