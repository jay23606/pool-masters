...
async function startDrill(id){
 ...
 showGame()
 $('#game').classList.add('focus');$('.call-actions').hidden=true;$('#room-sidebar').hidden=true
 ...
 state.game=new PoolGame({drill:d,drillLabel:`${DRILLS.indexOf(d)+1} OF ${DRILLS.length}`,mode:'8ball',renderer:await ensureRenderer(),surface:$('.canvas-wrap'),status:$('#game-status'),groupStatus:$('#groups'),callout:$('#callout'),power:$('#power'),powerOut:$('.shot-controls output'),shoot:$('#shoot'),spinPad:$('#spin'),moveCue:$('#move-cue'),changePocket:$('#change-pocket'),sfx,onReplay:paintReplayBar,onDrill:onDrillResult,aimSensitivity:tablePrefs.aimSensitivity,host:true,practice:false,spectator:false,send:()=>{}})
 state.game.setReady(true)
 $('#drill-bar').hidden=false;$('#drill-next').hidden=true
}
