...
function renderRooms(rooms){$('#rooms').innerHTML=rooms.length?rooms.map(r=>{const seats=r.metadata?.seats||{},count=1+(seats.b?1:0);return `<div class="room"><button data-code="${r.code}"><span><b>${esc(r.name||r.hostName+"'s table")}</b><small>${esc(r.hostName)} · ${count}/2 playing</small></span><strong>${r.code}</strong></button><button class="watch" data-watch="${r.code}">Watch</button></div>`}).join(''):'<div class="empty">No open tables yet.<br>Create one or practice while you wait.</div>'}
