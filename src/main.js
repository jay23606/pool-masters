...
async function startHotSeat(mode=chosenMode()){
 ...
 state.game=new PoolGame({mode,renderer:await ensureRenderer(),surface:$('.canvas-wrap'),status:$('#game-status'),groupStatus:$('#groups'),callout:$('#callout'),power:$('#power'),powerOut:$('.shot-controls output'),shoot:$('#shoot'),spinPad:$('#spin'),moveCue:$('#move-cue'),changePocket:$('#change-pocket'),sfx,onReplay:paintReplayBar,aimSensitivity:tablePrefs.aimSensitivity,host:true,practice:true,hotSeat:true,names:{a:'Player 1',b:'Player 2'},send:()=>{},
