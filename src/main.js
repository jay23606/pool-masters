...
function startPractice(){state.game?.destroy();state.mode='practice';state.room=null;state.opponent={name:'AI Coach'};showGame();history.replaceState({},'',location.pathname);$('#room-label').textContent='Unranked practice';$('#versus').innerHTML=`<span><b>${esc(foyer.player.name)}</b><small>You</small></span><i>vs</i><span><b>AI Coach</b><small>Practice</small></span>`;renderAiRecord();state.game=new PoolGame({canvas:$('#table'),status:$('#game-status'),callout:$('#callout'),power:$('#power'),powerOut:$('.shot-controls output'),shoot:$('#shoot'),host:true,practice:true,send:()=>{},onFinish:result=>{recordAiResult(result.winner==='a');$('#next-rack').hidden=false}});$('.call-actions').hidden=true;$('#video-panel').hidden=true;$('.chat').hidden=true}
function showGame(){$('#lobby').classList.remove('active');$('#game').classList.add('active');$('#messages').innerHTML='';$('#video-panel').hidden=false;$('.chat').hidden=false;$('#practice-record').hidden=true;$('#next-rack').hidden=true}
