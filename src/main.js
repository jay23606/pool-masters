...
async function buildRenderer(){
 if(view.mode==='3d'){
  try{const{createRenderer3D}=await import('./render3d.js');return await createRenderer3D($('#table3d'))}
...
async function enterRoom(room){
 ...
 state.game=new PoolGame({renderer:await ensureRenderer(),surface:$('.canvas-wrap'),status:$('#game-status'),groupStatus:$('#groups'),callout:$('#callout'),power:$('#power'),powerOut:$('.shot-controls output'),shoot:$('#shoot'),spinPad:$('#spin'),host:room.isHost,practice:false,send:broadcastGame,onFinish:finishRanked})
